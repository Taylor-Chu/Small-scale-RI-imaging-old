...
from .mrop_ri_measurement_operator.src.utils.solve_epsilon import solve_epsilon_same_aa
from .ri_measurement_operator.pysrc.measOperator.meas_op_nufft_pytorch_finufft import (
    MeasOpPytorchFinufft,
)

from .mrop_ri_measurement_operator.src.mrop_vmap_mf_mod_KB_same_aa import (
    create_meas_op_ROP_vmap_mf_mod_KB_same_aa as create_meas_op_ROP,
)
...
def imager(param_optimiser: Dict, param_measop: Dict, param_proxop: Dict) -> None:
    ...
    print(
        f"INFO: Original dimensions: N = {N}, Q = {Q}, K = {K}, B = {B}, N_ratio = {param_measop["ROP_param"]["N_ratio"]}."
    )
    epsilon, P_Q, M_B, M_K = solve_epsilon_same_aa(
        N,
        param_measop["ROP_param"]["Q"],
        B,
        K,
        param_measop["ROP_param"]["N_ratio"],
        param_measop["ROP_param"]["epsilon_n"],
    )

    print(f"INFO: Calculated epsilon for MROP modulation dimensions: {epsilon:.4f} for alpha=beta, by solving the polynomial 0 = e^4 - e^3/Q - 2N/n^2Q^2QK")
    ...
    print(
        f"INFO: PM / N = {param_measop["ROP_param"]["P"] * (param_measop["ROP_param"]["P"] - 1) // 2 * param_measop["ROP_param"]["M"] / N:.4f}",
        flush=True,
    )
